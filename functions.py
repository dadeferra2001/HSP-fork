...
def get_dataset(filename, test_size = 0.2, random_state = RANDOM_STATE):
    ...
    if os.path.isfile(os.path.join('cleaned', f"cleaned_{filename}")):
        ...
        df = pandas.read_csv(os.path.join(os.getcwd(), 'cleaned', f'cleaned_{filename}'))
        ...
        df = pandas.read_csv(os.path.join(os.getcwd(), 'original', filename))
        ...
        df.to_csv(os.path.join(os.getcwd(), 'cleaned', f'cleaned_{filename}'), index = False)

        # Return only the wanted one
        return df
    

# Function that train a model using HuggingFace Trainer helper class
def train(model_name, model, tokenizer, batch_size, epochs, train_encoded, test_encoded):
    training_args = TrainingArguments(output_dir=os.path.join('checkpoints', model_name), save_steps = 5000, per_device_train_batch_size=batch_size, per_device_eval_batch_size=batch_size, auto_find_batch_size = True, num_train_epochs=epochs)
    trainer = Trainer(model=model, args=training_args, train_dataset=train_encoded, eval_dataset=test_encoded, tokenizer=tokenizer, compute_metrics=lambda pred: {"accuracy": accuracy_score(pred.label_ids, pred.predictions.argmax(-1))})
